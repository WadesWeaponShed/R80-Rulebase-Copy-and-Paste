...
mgmt_cli -d $DOMAIN -s id.txt show access-layers limit 500 --format json --conn-timeout | jq --raw-output '."access-layers"[] | (.name)'
...
printf "\nExport Started\n"
mgmt_cli -d $DOMAIN -s id.txt show access-rulebase name "$POL_NAME" limit 1500 details-level full --conn-timeout 3600 --format json >> "$POL2".json
